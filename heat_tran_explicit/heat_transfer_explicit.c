...
int main(int argc, char** args) {
	...
	PetscReal      err, fvalue, norm_uold = 0.0, norm_u = 1.0;  /* norm of solution error */
	PetscErrorCode ierr;
	PetscInt       cc, col[3], maxit = 5000, rstart, rend, rlocal, clocal;
	PetscInt       max = 50000;
	PetscInt       n = N;
	PetscScalar    a = 1.0, value[3];
	...
	*/
	...
	for (int iter = 0; iter < maxit; iter++) {
		MatCopy(U, Uold);
		MatMatMult(A, Uold, MAT_REUSE_MATRIX, fill, U);
		MatAXPY(U, a, U, SAME_NONZERO_PATTERN);
